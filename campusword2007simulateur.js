...
(function () {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // =========================================
        // SAFE EDITOR DETECTION
        // =========================================

        const editor =
            document.querySelector(".page-text-layer") ||
            document.querySelector("[contenteditable='true']");

        if (!editor) {
            console.warn("Editor pa jwenn - Home Tab pa aktive.");
            return;
        }
        ...
        if (btnCut) btnCut.addEventListener("click", () => exec("cut"));
        if (btnCopy) btnCopy.addEventListener("click", () => exec("copy"));
        if (btnPaste) btnPaste.addEventListener("click", () => exec("paste"));

        // Format Painter (simulation)
        if (btnFormatPainter) {
            btnFormatPainter.addEventListener("click", () => {
                alert("Format Painter (simulation) - feature coming soon");
            });
        }
        ...
        map.forEach(([id, cmd]) => {
            ...
            if (btn) btn.addEventListener("click", () => exec(cmd));
        });

        // =========================================
        // 4. FONT SIZE GROW / SHRINK
        // =========================================

        const grow = document.getElementById("btn-grow-font");
        const shrink = document.getElementById("btn-shrink-font");
        ...
        const caseBtn = document.getElementById("btn-change-case");
        ...
        const clear = document.getElementById("btn-clear-formatting");
        if (clear) {
        ...
        paragraphActions.forEach(([text, cmd]) => {
            const btn = Array.from(document.querySelectorAll(".ribbon-btn"))
                .find(b => b.textContent.trim() === text);

            if (btn) btn.addEventListener("click", () => exec(cmd));
        });

        // Lists (simple simulation)
        const listButtons = Array.from(document.querySelectorAll(".ribbon-btn"))
            .filter(b => b.textContent.includes("List"));

        listButtons.forEach(btn => {
            btn.addEventListener("click", () => exec("insertUnorderedList"));
        });

        // Indent
        const indentLeft = Array.from(document.querySelectorAll(".ribbon-btn"))
            .find(b => b.textContent.includes("← Indent"));

        const indentRight = Array.from(document.querySelectorAll(".ribbon-btn"))
            .find(b => b.textContent.includes("Indent →"));

        if (indentLeft) indentLeft.addEventListener("click", () => exec("outdent"));
        if (indentRight) indentRight.addEventListener("click", () => exec("indent"));

        // Show paragraph marks (simulation)
        const pilcrow = Array.from(document.querySelectorAll(".ribbon-btn"))
            .find(b => b.textContent.includes("¶"));

        if (pilcrow) {
            pilcrow.addEventListener("click", () => {
                alert("Paragraph marks (¶) simulation mode");
            });
        }
        ...
        const findBtn = Array.from(document.querySelectorAll(".ribbon-btn"))
            .find(b => b.textContent === "Find");

        const replaceBtn = Array.from(document.querySelectorAll(".ribbon-btn"))
            .find(b => b.textContent === "Replace");

        const selectBtn = Array.from(document.querySelectorAll(".ribbon-btn"))
            .find(b => b.textContent === "Select");

        if (findBtn) findBtn.addEventListener("click", () => alert("Find feature UI not implemented yet"));
        if (replaceBtn) replaceBtn.addEventListener("click", () => alert("Replace feature UI not implemented yet"));
        if (selectBtn) selectBtn.addEventListener("click", () => document.execCommand("selectAll"));

        // =========================================
        // 9. STYLE GROUP (SIMULATION ONLY)
        // =========================================

        const styleButtons = ["Normal", "No Spacing", "Heading 1", "Heading 2", "Title", "Subtitle"];
        ...
        styleButtons.forEach(style => {
            ...
            if (btn) {
                btn.addEventListener("click", () => {
                    alert(`Style applied: ${style} (simulation)`);
                });
            }
        });
        ...
        console.log("HOME TAB FULL ENGINE ACTIVATED - 100% SAFE MODE");
