...
// ========================
// 📦 GENERATE QR ABONNEMENT MERCHANT
// ========================
router.get("/generate-abonnement-qr", async (req, res) => {
  try {
    const email = req.query.email || req.body.email || "memeprojet@fobas.com";
    ...
    // Payload QR pou mwa
    const payloadMonth = JSON.stringify({
      type: "FOBAS_PRODUCT",
      merchantEmail: email,
      productId: "PROD-001",
      productName: "Abonnement Merchant /Mois",
      amount: 1000,
      currency: "HTG"
    });

    // Payload QR pou ane
    const payloadYear = JSON.stringify({
      type: "FOBAS_PRODUCT",
      merchantEmail: email,
      productId: "PROD-001",
      productName: "Abonnement Merchant /Annee",
      amount: 5000,
      currency: "HTG"
    });

    // Jenere URL QR
    const qrUrlMonth = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(payloadMonth)}`;
    const qrUrlYear  = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(payloadYear)}`;
    ...
    console.error("GENERATE ABONNEMENT QR ERROR:", err);
    return res.status(500).json({
      success: false,
      message: "Erreur serveur génération QR abonnement"
    });
  }
});
