...
document.addEventListener("DOMContentLoaded", () => {
  try {
    // ==== Verifye paj la se fòm expressfobas la ====
    if (!window.location.pathname.includes("expressfobas/expressfobas.html")) return;

    // ==== Chache eleman DOM dashboard la sou paj aktyèl la (si li egziste) ====
    const userNameEl = document.getElementById("userName");
    const userEmailEl = document.getElementById("userEmail");
    const userRoleEl = document.getElementById("userAccountType");

    if (!userNameEl || !userEmailEl || !userRoleEl) {
      console.warn("Dashboard DOM introuvable. Ranpli fòm otomatik skip.");
      return;
    }

    // ==== Rekipere done itilizatè yo soti nan dashboard ====
    let userName = userNameEl.textContent.trim();
    let userEmail = userEmailEl.textContent.trim();
    let userRole = userRoleEl.textContent.trim();

    // Retire prefiks "Tit / Statut:" si li prezan
    if (userRole.includes(":")) {
      userRole = userRole.split(":")[1].trim();
    }

    // ==== Chache chan fòm yo nan paj expressfobas la ====
    const agentNameInput = document.getElementById("agentName");
    const agentEmailInput = document.getElementById("agentEmail");

    // ==== Ranpli chan yo sèlman si yo egziste ====
    if (agentNameInput) {
      agentNameInput.value = userName;
      agentNameInput.readOnly = true; // fè li readonly pou sekirite
    }

    if (agentEmailInput) {
      agentEmailInput.value = userEmail;
      agentEmailInput.readOnly = true;
    }

    console.log(`Fòm expressfobas ranpli otomatik: ${userName} | ${userEmail} | ${userRole}`);
  } catch (err) {
    console.error("Erè nan script ranpli fòm otomatik:", err);
  }
});
