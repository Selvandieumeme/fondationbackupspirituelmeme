...
<script>
// ==== Script ekstèn pou jwe mizik MEME otomatik ====

// Mizik default pou MEME (ou ka mete nenpòt URL mp3)
const MEME_DEFAULT_AUDIO = "https://fondationbackupspirituel.com/AudiofinalBilove";

// Fonksyon pou jwe fichye audio
function playMemeAudio(url){
  if(!url) return;
  const audio = new Audio(url);
  audio.volume = 0.8; // nivo volim, ajiste si ou vle
  audio.play().catch(err => console.warn("MEME audio pa ka jwe:", err));
}

// Fonksyon pou tcheke nouvo mesaj MEME nan chatbox
function observeMemeChat(){
  const chatBox = document.getElementById('chatMessages'); // chat DOM MEME
  if(!chatBox) return;
...
// ==== Opsyonèl: jwe mizik soti nan MEME_QA dirèkteman ====
function playAudioFromQA(question){
  if(!window.MEME_QA) return;
  const entry = window.MEME_QA.find(q => q.question.toLowerCase() === question.toLowerCase());
  if(entry && entry.audioFile){
    playMemeAudio(entry.audioFile);
  } else {
    // Sinon jwe mizik default
    playMemeAudio(MEME_DEFAULT_AUDIO);
  }
}

// Lanse observer otomatikman lè paj chaje
window.addEventListener('DOMContentLoaded', () => {
  observeMemeChat();
  console.log("MEME Audio observer aktif: chak nouvo mesaj pral jwe mizik otomatikman.");
});
</script>
