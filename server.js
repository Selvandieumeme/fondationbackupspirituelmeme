...
// ================================
// KONFIG API FOBAS — PA LOCALHOST
// ================================
const BASE_API = "https://api.fondationbackupspirituel.com";
...
// =============================================
// 4️⃣  ROUTES — VIRTUAL DEBIT CARD FOBAS (EMAIL-BASED)
// =============================================
...
// 🔵 Create new card
app.post("/cards/:email/create", async (req, res) => {
  const { email } = req.params;

  try {
    const now = new Date();
    const emiMonth = String(now.getMonth() + 1).padStart(2, "0");
    const emiYear = now.getFullYear();

    const EmiDate = `${emiMonth}/${emiYear}`;
    const ExpDate = `${emiMonth}/${emiYear + 3}`;

    const generatedCVV = String(Math.floor(100 + Math.random() * 900));
    const generatedNumber = "4598 " + Math.floor(Math.random()*9000+1000) + " **** " + Math.floor(Math.random()*9000+1000);
...
// 🔵 Load / Top-up card using walletbalances
app.post("/cards/:cardId/load", async (req, res) => {
  const { cardId } = req.params;
  const { amount, email } = req.body;

  try {
    // Tcheke balans WALLET itilizate a nan collection walletbalances
    const walletbalances = await WalletBalance.findOne({ email });

    if (!walletbalances) {
      return res.status(400).json({ error: "Wallet pa jwenn" });
    }

    if (walletbalances.balance < amount) {
      return res.status(400).json({ error: "Kantite lajan nan wallet la pa sifi" });
    }

    // Retire nan walletbalances
    walletbalances.balance -= amount;
    walletbalances.lastUpdate = new Date();
    await walletbalances.save();
    ...
    await CardTx.create({
      ...
      amount,
      type: "credit",
      description: "Recharge kat virtuel FOBAS"
    });

    res.json({ success: true, message: "Kat la chaje", cardBalance: card.balance });

  } catch (err) {
    res.status(500).json({ error: "Echèj chajman kat la", details: err.message });
  }
});
...
app.get("/cards/:cardId/fobas_card_tx", async (req, res) => {
  ...
  try {
    const tx = await CardTx.find({ cardId }).sort({ createdAt: -1 });
    res.json(tx);
