...
async function getCollections() {
  ...
  return {
    walletUsers: db.collection('walletusers'),
    walletBalances: db.collection('walletbalances')
  };
}

// ------------------ 1️⃣ Aktive Abonnement ------------------
router.post('/subscribeCashback', async (req, res) => {
  try {
    const { userEmail } = req.body;
    if (!userEmail) return res.status(400).json({ success: false, message: "Email itilizate obligatwa." });

    const { walletUsers, walletBalances } = await getCollections();

    const user = await walletUsers.findOne({ email: userEmail });
    if (!user) return res.status(404).json({ success: false, message: "Itilizate pa jwenn." });

    let userBalance = await walletBalances.findOne({ userId: user._id });
    if (!userBalance) {
      await walletBalances.insertOne({ userId: user._id, balance: 0, bonus: 0 });
      userBalance = { userId: user._id, balance: 0, bonus: 0 };
    }

    if (userBalance.balance < 250) return res.status(400).json({ success: false, message: "Balance itilizate twò ba pou abonman." });
    ...
    const admin = await walletUsers.findOne({ email: adminEmail });
    if (!admin) return res.status(404).json({ success: false, message: "Admin pa jwenn." });

    let adminBalance = await walletBalances.findOne({ userId: admin._id });
    if (!adminBalance) {
      await walletBalances.insertOne({ userId: admin._id, balance: 0, bonus: 0 });
    }
    ...
    return res.json({ success: true, message: "Abonnement ak cashback aktive avèk siksè." });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ success: false, message: "Erè serveur." });
  }
});

// ------------------ 2️⃣ Aplike Cashback 25G pou chak tranzaksyon ------------------
async function applyCashback(transactionType, userId) {
  try {
    const { walletUsers, walletBalances } = await getCollections();

    const user = await walletUsers.findOne({ _id: ObjectId(userId) });
    if (!user || !user.sponsorEmail) return;

    const sponsor = await walletUsers.findOne({ email: user.sponsorEmail.toLowerCase() });
    if (!sponsor) return;

    let sponsorBalance = await walletBalances.findOne({ userId: sponsor._id });
    if (!sponsorBalance) {
      await walletBalances.insertOne({ userId: sponsor._id, balance: 0, bonus: 0 });
      sponsorBalance = { userId: sponsor._id, balance: 0, bonus: 0 };
    }
    ...
    if (!adminBalance) {
      await walletBalances.insertOne({ userId: admin._id, balance: 0, bonus: 0 });
      adminBalance = { userId: admin._id, balance: 0, bonus: 0 };
    }

    if (adminBalance.balance >= 25) {
      await walletBalances.updateOne({ userId: admin._id }, { $inc: { balance: -25 } });
      await walletBalances.updateOne({ userId: sponsor._id }, { $inc: { balance: 25 } });
      console.log(`Cashback 25G soti admin -> ${sponsor.email} pou tranzaksyon ${transactionType}`);
    }
  } catch (err) {
...
cron.schedule('0 0 1 * *', async () => {
  try {
    ...
    const allUsers = await walletUsers.find({}).toArray();
    for (const user of allUsers) {
      let userBalance = await walletBalances.findOne({ userId: user._id });
      if (!userBalance || userBalance.balance < 250) continue;

      await walletBalances.updateOne({ userId: user._id }, { $inc: { balance: -250 } });
      await walletBalances.updateOne({ userId: admin._id }, { $inc: { balance: 250 } });
    }
    console.log("Cron premye mwa: 250G retire sou itilizate, ajoute sou admin.");
  } catch (err) {
    console.error("Erreur cron cashback:", err);
  }
});
